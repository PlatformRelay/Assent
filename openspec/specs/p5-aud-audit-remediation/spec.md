...
- ~~Given a subsequent commit that does NOT regenerate the changelog, when `task check` (or the
  ...
  the tag out of REQ-AUD-S03-01); `changelog_gate_test.sh` §1 warns there too, since CI runs
  `task check` in `release-exitgate`. Pinned by §10c and §10e at both polarities.
