...
- **REQ-E5-S07-03** *(SECURITY · D-129, closes OQ-28)* — **filesystem** containment, not only path containment. `RepoFileOpts.FS` must be a symlink-safe root (`(*os.Root).FS()` via `builtin.OpenRepoRoot`); `cmd/assent` injects one for the checkout tree, which is the MR head. Independently, a candidate whose path traverses a symlink at ANY component is refused (`unavailable`, contributor-readable reason naming the candidate) and STOPS the walk-up — it is never skipped to a less-specific file, and in-root symlinks are refused too because a link that stays inside the FS root can still leave the declared `roots` clip. Legitimate in-root resolution is unchanged. **Proof location (D-129 amendment, 2026-08-09):** D-133 refuses any symlink under the checkout's `base/`/`head/` at changed-file enumeration, BEFORE providers resolve, so this requirement is no longer falsifiable end-to-end through `assent run --checkout`; its cmd-level proof is `resolveRunFacts` — the function the run itself calls — over the same planted tree. Test: `internal/provider/builtin/repo_file_symlink_test.go`, `cmd/assent/run_provider_symlink_test.go` (`TestResolveRunFactsRefusesSymlinkedQuotaCandidate` pins the two layers separately; `TestRunCheckoutSymlinkRefusedAtEnumeration` pins the subsumption and reds when the refusal is folded opaque); Verify: `go test ./internal/provider/... -run TestRepoFileSymlink && go test ./cmd/assent/... -run 'TestRunCheckout|TestResolveRunFactsRefusesSymlinkedQuotaCandidate'`; Level: L0+L1
...
- **REQ-E5-S08-03** *(SECURITY · D-129 / D-130)* — the ownership registry decides who may approve, so it is a decision input: it loads from the **target ref** first, and the checkout is a fallback **only when the registry is genuinely ABSENT at that ref** — `errors.Is(err, forge.ErrNotFound)`, the neutral port sentinel, never `gitlab.ErrNotFound` (ARCH-02 / depguard). Any other forge error (503, 429, 401, a broken proxy) is **not** absence: it yields no client and no registry, and that error PROPAGATES out of `resolveRunFacts` — the run **aborts** with a non-zero exit, **no DecisionRecord and no forge write**. That is more conservative in what assent WRITES, but **not** strictly more conservative in effect: a rerun that aborts leaves any prior `✅ APPROVE` summary standing unrevised and posts no thread, whereas a REVIEW rerun upserts that same summary to REVIEW and adds an unresolved discussion — and that discussion is what holds the merge under `only_allow_merge_if_all_discussions_are_resolved`, the setting arming already requires (`internal/forge/precondition.go`). Measured note/thread counts and the arming-chain trace are in **D-130**. Falling back on any error was a fail-OPEN — a merely-unreachable forge promoted the merge request's own head tree to the who-may-approve authority with nothing surfaced — so "never a shadow" holds only with the absence gate in place (ADR-0015 §1 / GUIDELINES §Safety 2+3). A registry reached through a symlink is refused with an error too, never a partial load — and that refusal is a non-absence failure like any other above, so it propagates out of `resolveRunFacts` and aborts the run rather than leaving an unresolved fact behind. Test: `internal/provider/builtin/resource_owner_symlink_test.go`, `cmd/assent/provider_host_registry_test.go` (incl. `TestResourceOwnerRegistryTransientForgeErrorNeverFallsBackToCheckout`, `TestResourceOwnerRegistryForgeErrorAbortsResolveRunFacts`, which pins the `providerCallFor` → `resolveRunFacts` propagation, and `TestResourceOwnerDeclarationResolvesOwnerFact`, which pins the HEALTHY resolve so the refusal tests above cannot all pass against a permanently-invalid fact); Verify: `go test ./internal/provider/... -run TestResourceOwner && go test ./cmd/assent/... -run TestResourceOwner`; Level: L0+L1
