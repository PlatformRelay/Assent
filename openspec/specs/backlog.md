...
| GATES3-F01 | `hack/audit/README.md:214` publishes the AUD2 control floor as **"38 full, 32 text-only"**; the code says **57 / 51** (`hack/audit/aud2_exitgate_test.sh:2410`). The published number stopped matching the gate when D-157 raised the floor twice, and it was already stale before that (last touched in `cc25f20`) | **[autonomous]** | — | found *by* the GATES3 lane's final review, deliberately NOT fixed there: `hack/audit/README.md` is outside that lane's fence, and a silent edit to a published number is the same D-124 species as the drift itself — a doc claim nothing grades. Worth a row that also asks whether the floor should be asserted against the README rather than restated in it |
