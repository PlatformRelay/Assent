...
| --- | --- | --- | --- | --- | --- |
| **SONAR-SHELL** | Shell-script hygiene in `hack/*.sh` (`validate-schemas-stock.sh` 19 · `check-sanitization.sh` 18 · `check-migration-invariants.sh` 13 · `spikes/e2e/smoke.sh` 4) | `shelldre:S7688` use `[[` not `[` (34) · `S7679` assign positional param to local (9) · `S7682` explicit `return` (5) · `S131` add default `*)` case (3) · `S1192` de-dup literal (2) · `S7684` lower_case var (1) = **54** | **these are CI-gate scripts** — mechanical but must stay byte-behaviour-identical; re-run each script as the gate | **DONE** (2026-09-05, [D-171](../../docs/decisions/decisions.md)) — **the 54 count was stale on the day it was written**: 52 of 54 were cleared by `9a772d4` (2026-08-03 22:23), which landed *after* the 2026-08-03 SonarCloud snapshot this row was derived from. Re-measured against `20c80cb`: 0 residual `S7679`/`S7682`/`S131`/`S1192`/`S7684`, and 2 residual `S7688` that are **not** part of the 54 — they arrived later with `895ff7e` (BASH32-F01/D-154) at `hack/validate-schemas-stock.sh:55-56`. Those two are deliberately left: they are one copy of a three-site guard idiom whose canonical spelling is the USAGE block in `hack/lib/require-bash.sh`, with siblings in `hack/release/verify-artifacts.sh` and `hack/docs/truthlag_pins_test.sh` — converting one copy alone desynchronises the idiom from its own documentation, so it is dispatched as a follow-up rather than done piecemeal. What this lane added is the evidence nobody had: a 59-check differential run of all four gates in three versions (pre-`9a772d4`, post, and HEAD) over happy-path *and* negative inputs, all identical, plus 4 mutants proving those checks are non-vacuous | no |
