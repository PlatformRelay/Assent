...
| GATES3 | **DONE** (D-157) — the three PR-visible text gates (`hack/lint/workflow_pins_test.sh`, `hack/audit/aud2_exitgate_test.sh`, `hack/examples/dogfood_wiring_test.sh`) now share `hack/lib/pr_reach.sh`: PR reach is graded structurally (a `paths:`/`paths-ignore:`/narrowing-`types:`/main-excluding-branch filter, a `pull_request:` nested under another key, or a job-level `if:`/`needs:` all red), each gate **cross-pins the other two's steps**, and aud2's fixed-string step check is replaced by the anchored `run:`-command form | **[autonomous]** | — | three reviewer-demonstrated residuals, all in the same three files, all deliberately left unfixed on their own lanes because fixing one of three identical checks would have been misleading |
