...
| --- | --- | --- | --- | --- | --- |
| **SONAR-SHELL** | Shell-script hygiene in `hack/*.sh` (`validate-schemas-stock.sh` 19 · `check-sanitization.sh` 18 · `check-migration-invariants.sh` 13 · `spikes/e2e/smoke.sh` 4) | `shelldre:S7688` use `[[` not `[` (34) · `S7679` assign positional param to local (9) · `S7682` explicit `return` (5) · `S131` add default `*)` case (3) · `S1192` de-dup literal (2) · `S7684` lower_case var (1) = **54** | **these are CI-gate scripts** — mechanical but must stay byte-behaviour-identical; re-run each script as the gate | **DONE** (2026-09-05, [D-171](../../docs/decisions/decisions.md)) — **the 54 count was stale 4h17m after it was written**: `c2a6ab7` introduced this row at 18:06 on 2026-08-03 from a live query taken earlier that day, and `9a772d4` cleared **54 of 54** at 22:23 the same evening; the row then said `OPEN` for 33 days. Measured at `9a772d4`: 34/34 `S7688`, 9/9 `S7679`, 3/3 `S131`, 1/1 `S7684` and both remaining families cleared, zero residual in any of the six. The 2 `S7688` sites outstanding today at `hack/validate-schemas-stock.sh:55-56` are a **separate, later population** — `895ff7e` (BASH32-F01/D-154) introduced them 20 days afterwards — and are tracked as **SONAR-SHELL-R01** (below), not folded into this row's count. This lane changed no script; it supplied the evidence nobody had, since `9a772d4`'s "byte-identical behaviour" claim about four CI-gate scripts had never been tested. See D-171 for the differential run, its four mutants, and a marked note withdrawing one unfalsifiable figure in that evidence | no |
