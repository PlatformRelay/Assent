...
| **AUD-AGENTS-FRESH** | Pin `AGENTS.md` freshness: assert the hard-rule count matches what the tree cites by number (`.golangci.yml`, `internal/core/purity_test.go`, `hack/audit/*_exitgate_test.sh`, `hack/docs/truthlag_pins_test.sh` and four `openspec/specs/**` docs cite "rule N" — 27 citations across 13 files), and that *Current phase* still defers to `meta-plan.md` instead of restating per-epic status | **OPEN** `[autonomous]` | no | Residual of **D-176** / DOC-03. Tracking the file made it reviewable, not gated — a citation to "rule 7" resolving to nothing, or a re-duplicated status table going stale again, both pass every gate today. Both claims are mechanically checkable |
