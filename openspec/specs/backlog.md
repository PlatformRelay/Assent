...
| ID | Item | Status | Needs operator | Notes |
| --- | --- | --- | --- | --- |
| **E11-R01** | ADR-0002's `rego` bullet called the tier an "escape hatch for **cross-entry checks**" — inaccurate for the shipped input contract: E11-S01 / D-156 strikes cross-manifest, and the tier's real justification is folds + unbounded graph reachability | ✅ **DONE ([D-166](../../docs/decisions/decisions.md))** | no (agent lane) | Closed **by this residual, not by E11-S12**, which nominally owned it under REQ-E11-S12-01: that is story 12 of 14 and its `Test:` list omitted `docs/adr/0002-*`, so if E11 never ran the ADR stayed wrong indefinitely. The bullet now states the two measured shapes and **Amendment 1** records the withdrawn sentence verbatim with the reason each of its three clauses failed. **What is narrowed but NOT closed** (D-166 says so in its own words, and it is why this row keeps a note rather than vanishing): `docs/adr/0002-*` is now named in REQ-E11-S12-01's `Test:` list, but nothing machine-consumes a REQ's `Test:` list, and the only gate that reads ADRs at all (`hack/docs/truthlag_pins_test.sh`) compares Status rows against `docs/adr/README.md` and never looks at body content — **so a regression re-inserting the withdrawn sentence still fails nothing**. A pin over the phrase would live in `hack/docs/`; D-166 was not scoped to write it and neither is [D-174](../../docs/decisions/decisions.md), which reconciled this row. Evidence: `docs/planning/rego-tier-ceiling.md` §3/§6 |
