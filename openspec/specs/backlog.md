...
stands. It added no stage: `CHECK_STAGES` was unchanged at 19 *by that fix*, because it extends the
existing `release-install-cosign-pin-test` gate instead of starting a second published truth
(D-128); the array has since moved on — BASH32-F01/D-154 took it to 20 and ORPHAN/D-159 to 21,
so read "19th stage" above as the ordinal at AUD2 time, not a live count. All three traps above were handled: the new sections are offline against the same
