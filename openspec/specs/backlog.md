...
| GATES3 | **DONE** (D-157) — the three PR-visible text gates (`hack/lint/workflow_pins_test.sh`, `hack/audit/aud2_exitgate_test.sh`, `hack/examples/dogfood_wiring_test.sh`) now share `hack/lib/pr_reach.sh`: PR reach is graded structurally (a `paths:`/`paths-ignore:`/narrowing-`types:`/main-excluding-branch filter, a `pull_request:` nested under another key, or a job-level `if:`/`needs:` all red), each gate **cross-pins the other two's steps**, and aud2's fixed-string step check is replaced by the anchored `run:`-command form. Independent review then found two P1s by mutating the REAL `verify.yaml` end-to-end (both fixed here): the filter greps were pinned to **exactly four spaces**, so a 6-space `paths:` — valid YAML, identical mapping — made all three gates green on the very shape the lane closes; and the step disarm grep lacked the sequence-item alternative, so `      - if: …` as a step's **first** key graded WIRED (that regex was inherited from `main`, not introduced) | **[autonomous]** | — | three reviewer-demonstrated residuals, all in the same three files, all deliberately left unfixed on their own lanes because fixing one of three identical checks would have been misleading |
