...
| **AUD-AGENTS-FRESH** | Resolve `AGENTS.md` citations and pin its shape: every `AGENTS.md rule N` reference in the tree must resolve to a rule that actually exists in the file, and the *Current phase* section must keep deferring to `meta-plan.md` rather than restating per-epic status | **OPEN** `[autonomous]` | no | Residual of **D-176** / DOC-03. `hack/audit/exitgate_test.sh` already enforces exactly this resolve-the-citation property for `D-nnn`, `AUD-Snn` and `REQ-AUD-Snn-nn` — there is no equivalent for `AGENTS.md rule N`, so a citation to a rule that was renumbered or deleted passes every gate today, as does a re-duplicated status table quietly going stale again. By the repo's own standard a gate should have caught the untracked-rulebook situation, and none did. **No count is published in this row on purpose:** the occurrence figure moves whenever any doc quotes the search pattern — D-176's own text does exactly that — so the gate must DERIVE the number rather than compare against one pinned in prose |
| **AUD-SONAR-REQUIRED** | Make the `verify` job's SonarCloud scan a **required** status check on `main`, so a missing or failed analysis blocks a merge rather than merely showing red | **OPEN — operator** | **yes** | Residual of **D-177**. Same class as `AUD-RELSE-08`: live branch-protection config, unreachable from in-tree. Until it is set, the scan is visible but non-blocking — better than the month of silence D-177 ended, weaker than the gate matrix. Note the two named gaps first: the step is skipped on fork PRs (D-177) and on Dependabot-triggered runs (D-178), because neither carries `SONAR_TOKEN`, so making it required without a decision on both would block every fork and Dependabot PR |
| **AUD-DISPO-OWNER** | `check_disposition_table` resolves "the named decision row mentions this finding", not "the CORRECT row is named" — give a finding's owning decision row a machine-readable ownership marker so exactly one row can claim it, checkable in both directions | **OPEN** `[autonomous]` | no | Found by independent review of the D-176 lane, 2026-09-05. Reverting Appendix-B's DOC-03 owner from `D-176` back to `D-132` leaves `hack/audit/exitgate_test.sh` **green**, because D-132 still contains the literal `DOC-03` inside its struck clause. **Not closable by better prose-matching** — requiring the id outside a `~~struck~~` span fails here, since `**DOC-03**` sits outside the strike while only the rationale is struck. D-176's in-place amendment made the hole *reachable* but did not create it, and should not be reversed: the gate has always enforced the weaker property. Scope this to a structured marker, not to teaching the gate about prose |
...
| --- | --- | --- | --- | --- | --- |
| **SONAR-SHELL** | Shell-script hygiene in `hack/*.sh` (`validate-schemas-stock.sh` 19 · `check-sanitization.sh` 18 · `check-migration-invariants.sh` 13 · `spikes/e2e/smoke.sh` 4) | `shelldre:S7688` use `[[` not `[` (34) · `S7679` assign positional param to local (9) · `S7682` explicit `return` (5) · `S131` add default `*)` case (3) · `S1192` de-dup literal (2) · `S7684` lower_case var (1) = **54** | **these are CI-gate scripts** — mechanical but must stay byte-behaviour-identical; re-run each script as the gate | **DONE** (2026-09-05, [D-171](../../docs/decisions/decisions.md) then corrected by [D-173](../../docs/decisions/decisions.md)) — **`9a772d4` cleared 53 of 54, not 54 of 54.** The *54 of 54* published by D-171 is **WITHDRAWN**; it was inferred from `9a772d4`'s diff for the two families nobody could measure locally, and it was wrong for one of them. **Measured against SonarCloud on 2026-09-05** (`api/issues/search?componentKeys=PlatformRelay_assent&rules=shelldre:<key>&resolved=true|false`, anonymous): `S7688` **34/34** · `S7679` **9/9** · `S1192` **2/2** · `S131` **3/3** · `S7684` **1/1** · `S7682` **4 of 5** — 53 resolved, all `CLOSED FIXED` on 2026-08-03. **The 54th** is `shelldre:S7682` at `hack/check-sanitization.sh`, function `decode_b64` — *"Add an explicit return statement at the end of the function"*, issue `AZ-GUjW6PEuX9t168c38`, created **2026-07-21**, therefore part of the original 54 and not a later arrival. **Closed by this lane** (SONAR-SHELL-F01): `decode_b64` now ends in a bare `return`, proven behaviour-identical over an 11-case differential incl. the base64-decode fixtures, in two `base64` implementations — see D-173. Sonar will not confirm the closure until the project is re-analysed; its last analysis is **2026-08-04T08:01:09Z**. The `S7688` sites outstanding today are a **separate, later population** — `895ff7e` (BASH32-F01/D-154) introduced them 20 days after `9a772d4` — and are tracked as **SONAR-SHELL-R01** (below), not folded into this row's count | no |
...
| **SONAR-GO-CX-TEST** | ~34 `go:S3776` hits in `*_test.go` (table-driven suites legitimately long by line-count, low by risk) | `go:S3776` (~34) | tests only | **DONE** (2026-09-05, D-150 decided / **D-177** implemented) — exempted, not refactored: `sonar-project.properties` carries the `go:S3776` × `**/*_test.go` exclusion, pinned by `hack/lint/workflow_pins_test.sh` with a mutation control | no — decided |
| **SONAR-GO-MISC** | Small Go smells: `godre:S8184` comment blank imports (4) · `go:S1186` comment empty funcs (4) · `godre:S8196` single-method interface naming (2) · `go:S1135` resolve TODOs (2) · `godre:S8205` extract nested anon struct (1) · `go:S107` 8-param func (1) · `go:S1192` const for `"sha256:"` ×4 (1) = **15** | trivial, isolated | ✅ **DONE (D-172)** — all 15 were fixed by [`5970e97`](https://github.com/PlatformRelay/assent/commit/5970e97) on **2026-08-03 23:03 +0200**, ~5h after the plan itself (`c2a6ab7`, 18:06) landed; the row then read `OPEN` for 33 days. SonarCloud's own **2026-08-04 08:01Z** analysis — which *postdates* the fix — reports **0** across all seven rule families (`rules=` facet query, positive control `go:S3776`→61 on the same filter). ⚠️ **That is also SonarCloud's LAST analysis: no scan has run since 2026-08-04** (`analysisDate` = 2026-08-04T08:01:09Z), so every SONAR-* count in this section is a month stale — see D-172 | no |
...
**Config note — SUPERSEDED 2026-09-05 by D-177; nothing above it blocks anything any more.** The
operator took the CI-based path: `sonar-project.properties` is checked in, the scanner runs as a
SHA-pinned step in `verify.yaml` after the coverage gate, and `SONAR_TOKEN` was already a repo
secret (added `2026-08-04T08:30:33Z`). The mutual exclusivity the struck note describes is real
and is now resolved in the config file's favour — **but it was already resolved in practice, and
badly:** Automatic Analysis was switched off on/around **2026-08-04** and the scanner half was
never built, so the project ran from `2026-08-04T08:01:09Z` to 2026-09-05 with **no analysis of
any kind**. Every figure in the snapshot above, and every count in the four lanes below, dates
from at or before that moment. **Re-measure before acting on any of them** — two of these rows
(SONAR-SHELL, SONAR-GO-MISC) were already stale when claimed and each cost a lane to discover.
The residual operator action is `AUD-SONAR-REQUIRED` below: the scan is now visible on PRs but is
not a *required* check, which is live branch-protection config and cannot be set from in-tree.

**Suggested order (2026-09-05: only SONAR-GO-CX-PROD is left open; the other three are closed):**
SONAR-SHELL → SONAR-GO-MISC → SONAR-GO-CX-PROD (real refactor value) → SONAR-GO-CX-TEST (closed
by exemption, D-177). None gate a release; all are startable once the operator wants to spend cycles on
...
| ID | Item | Status | Needs operator | Notes |
| --- | --- | --- | --- | --- |
| **SONAR-SHELL-R01** | The D-154 bash-floor guard bootstrap still spells its tests with POSIX `[`. **Measured 2026-09-05 with `shellcheck 0.11.0 -o all -s bash`, rule `SC2292` (the `shelldre:S7688` analogue): 12 occurrences on 10 distinct lines** across the four files — `hack/lib/require-bash.sh` **6 occurrences on 4 lines** (75 ×2, 81, 97, 98 ×2), `hack/validate-schemas-stock.sh:55-56` (2), `hack/release/verify-artifacts.sh:21-22` (2), `hack/docs/truthlag_pins_test.sh:45-46` (2). They must move **together**. **This row previously published `2`, and named `require-bash.sh:40-41` — a commented USAGE block — as that file's only involvement; the file's six *executable-body* occurrences were missed entirely, so the scope was understated in kind, not only in count** ([D-173](../../docs/decisions/decisions.md)) | **OPEN** | no (agent lane) | Not part of SONAR-SHELL's 54 — `895ff7e` introduced these 20 days after `9a772d4` cleared that set. **Instrument caveat:** 12/10 is a **shellcheck** count, not a live `shelldre:S7688` query; the two correspond but are not the same instrument, and SonarCloud's last analysis of this project is **2026-08-04T08:01:09Z**, so a Sonar figure for `S7688` is a month stale and shellcheck is the better evidence today. The lane that takes this should re-measure and re-state the instrument. Converting one copy alone puts the idiom at odds with its own canonical form in `require-bash.sh`'s USAGE block, for a non-gating maintainability win, so the piecemeal fix was deliberately declined (D-171). `[[` is bash 2.02+, so the conversion does not raise the effective floor these lines exist to assert; the operands are already quoted, so `[[` changes neither splitting nor globbing. The lane that takes this must re-run `hack/lint/bash_version_guard_test.sh`, whose mutation controls delete the block as the range `/^_assent_lib=/,/^require_bash[[:space:]]/` — anchors `[[` does not disturb. A second, larger candidate lives here: this lane's differential harness was never committed (see D-171's marked note), and `hack/lint/` is where it would become a standing gate |
...
`.github/workflows/verify.yaml`): one invocation over the S01 cassettes, `task check` across every
stage it pins incl. `changelog-verify` (no count: `CHECK_STAGES` is the single source and this
sentence said "14" while the array held 21 — DOCTRUTH / D-174), the RELSE-05 release gate, the DOC truth pins, the determinism
...
| ID | Story | Execution | Depends on | Gate contribution |
| --- | --- | --- | --- | --- |
| E10-S00 | **DONE** (D-155) — ⚠️ GitHub addressing & representation model: [github-addressing-model.md](../../docs/planning/github-addressing-model.md) answers all four questions, mints **12** conformance case IDs (each fail-closed case paired with a positive control) and **wires them into S01/S07/S14's DoD** rather than leaving them in a doc nothing reads, corrects **2 stale ADR-0021 anchors** (`provider_host.go:275→:292` — the D-130 registry — and `Describe→GetMR`) plus **1 in this epic's own spec** (`spec.md:257`, `provider_host.go:246→:263`, fixed here), and raises **OQ-33** (`protected-pipeline-source`) + **OQ-34** (adapter-computed CODEOWNERS eligibility). Both arming-relevant GitHub capabilities land `unknown`, so **v1 GitHub comments and does not gate** — fail-closed and stated up front. `git diff schemas/ == 0` holds — no schema change proposed. **Awaiting maintainer LGTM** | **[autonomous · design · LGTM]** | none | **do first** — kills both P0s before the port freezes |
| E10-S01 | **DONE** (D-165) — conformance suite extracted into importable Go: `RunSuite(t, Factory)`, explicit `Observer`/`Fixture` surfaces, and three self-checks (observed-execution catalog match, can-fail-against-sabotage per adapter, per-observation load-bearing). Found + closed: the two SHA-guard cases had never run against GitLab despite `forge: gitlab` | **[autonomous]** | S00 | no assertion weakened — several strengthened; all 5 cases now run on both adapters |
...
| E11-S01 | ✅ **DONE (D-156)** — tier-1 (CEL) ceiling recorded: `docs/planning/rego-tier-ceiling.md` | **[autonomous]** | none | **E11 NARROWED**: cross-manifest + set-difference **struck** (both unconditional). **Two justifications survive, both unconditional**: fold/aggregate, and graph reasoning — the number of nested iteration **levels** in a CEL expression is *syntactic*, fixed by the expression text (and capped at cel-go's 250), so **unbounded** reachability has no spelling (*amended 2026-09-05, [D-174](../../docs/decisions/decisions.md)* — this read, verbatim, "a CEL expression's iteration count cannot be data-dependent (depth is syntactic, capped at cel-go's 250), so **unbounded** reachability has no spelling", which [D-166](../../docs/decisions/decisions.md) F4-03 measured **false** and withdrew: a comprehension over an input collection costs `|N|`, and `|N|` is data. The verdict is unchanged; only the reason it rests on is. Amended in place rather than left standing because a row marked `✅ DONE` gives a reader no cue that anything in it is outstanding — which is what made this the more misleading of the two survivors D-166 handed on). A bounded `k`-hop check *is* writable and on a small graph is affordable and even complete — the ceiling is expressive, not performance. Cross-manifest is an *input* limit S05's identical `EvaluationInput` does not lift. Sets an S04 allowlist floor (`graph.reachable`, plus `split` as a convenience) held by **review**, not by REQ-E11-S04-02's drift-only golden. Residuals OQ-35/OQ-36 gate nothing |
...
| E11-S03 | Module loading from the **target ref**; compile failure is a lint hard error. **UNBLOCKED by [D-144](../../docs/decisions/decisions.md) (2026-08-16): rule-7 answered (d1)** — the narrowing is accepted explicitly, so this story may add OPA to `go.mod` inside the guarded tree | **[autonomous · engine-grade · LGTM]** | **S02, and REQ-E11-S04-03's transitive `go list -deps` purity check must land FIRST (D-144)** | no second, laxer load path |
| E11-S04 | OPA capability sandbox — denies `http.send`/`time.now_ns`/`rand.intn` via a golden allowlist an OPA upgrade cannot widen unnoticed. **UNBLOCKED by [D-144](../../docs/decisions/decisions.md): mechanism is (d1)**, evaluator stays in `internal/core/policy`, ADR-0011 amended to a behavioural (capability-enforced) guarantee | **[autonomous · engine-grade · LGTM]** | S03 | both purity gates are non-transitive today — see D-141/D-144 |
...
**E11 residuals, tracked independently of the epic** — because they concern a *published* ADR and
must not wait on 14 stories that may never run:

| ID | Item | Status | Needs operator | Notes |
| --- | --- | --- | --- | --- |
| **E11-R02** | Re-inserting the sentence [D-166](../../docs/decisions/decisions.md) withdrew from ADR-0002's `rego` bullet — "escape hatch for cross-entry checks, complex derivations, whole-branch conventions" — **fails nothing**. D-166 named two reasons E11-R01 was ungatable and closed only the first: `docs/adr/0002-*` is now in REQ-E11-S12-01's `Test:` list, but **nothing machine-consumes a REQ's `Test:` list** — no script, workflow or Go test parses those lines — and the only gate that reads ADRs at all, `hack/docs/truthlag_pins_test.sh`, compares each ADR's Status row against `docs/adr/README.md` and never looks at body content | **OPEN** | no (agent lane) | Split out of E11-R01 by [D-174](../../docs/decisions/decisions.md) rather than left inside it: a residual that lives only in a row which has just flipped to ✅ DONE loses its tracking slot with it — [D-171]'s words for the anti-pattern, and D-166 recorded that exact failure *about E11-R01*. The pin belongs in `hack/docs/`, reached through the existing `docs-gates` stage; it was outside D-166's fence and outside D-174's. Scope is one phrase-absence assertion over `docs/adr/0002-*` plus its mutation control (the withdrawn sentence re-inserted must red), which is the shape `truthlag_pins_test.sh` already uses |
| **E11-R01** | ADR-0002's `rego` bullet called the tier an "escape hatch for **cross-entry checks**" — inaccurate for the shipped input contract: E11-S01 / D-156 strikes cross-manifest, and the tier's real justification is folds + unbounded graph reachability | ✅ **DONE ([D-166](../../docs/decisions/decisions.md))** | no (agent lane) | Closed **by this residual, not by E11-S12**, which nominally owned it under REQ-E11-S12-01: that is a late story of an epic that may never run, and its `Test:` list omitted `docs/adr/0002-*`, so the ADR would have stayed wrong indefinitely (no story ordinal or total — E11's story count is graded nowhere either, D-174). The bullet now states the two measured shapes and **Amendment 1** records the withdrawn sentence verbatim with the reason each of its three clauses failed. **The half D-166 narrowed but did NOT close is `E11-R02` above, as its own OPEN row** — re-inserting the withdrawn sentence still fails nothing, and a residual tracked from inside a row marked DONE loses its slot with it ([D-174](../../docs/decisions/decisions.md)). Evidence: `docs/planning/rego-tier-ceiling.md` §3/§6 |
...
| GATES3 | **DONE** (D-157) — the three PR-visible text gates (`hack/lint/workflow_pins_test.sh`, `hack/audit/aud2_exitgate_test.sh`, `hack/examples/dogfood_wiring_test.sh`) now share `hack/lib/pr_reach.sh`: PR reach is graded structurally (a `paths:`/`paths-ignore:`/narrowing-`types:`/main-excluding-branch filter, a `pull_request:` nested under another key, or a job-level `if:`/`needs:` all red), each gate **cross-pins the other two's steps**, and aud2's fixed-string step check is replaced by the anchored `run:`-command form. Independent review then found two P1s by mutating the REAL `verify.yaml` end-to-end (both fixed here): the filter greps were pinned to **exactly four spaces**, so a 6-space `paths:` — valid YAML, identical mapping — made all three gates green on the very shape the lane closes; and the step disarm grep lacked the sequence-item alternative, so `      - if: …` as a step's **first** key graded WIRED (that regex was inherited from `main`, not introduced). Two further review rounds each found one more P1 **in the previous round's fix**: a block sequence written FLUSH with its key was misgraded and the new key allowlist turned one such legitimate case into a red, and branch names were tokenised on `-`/`/` so `branches: [main-next]` yielded a `main` token and a main-EXCLUDING filter reported "runs on every PR"; then a stray **CR** on one sequence item made the same key report the same thing again. Five P1s total, three of them introduced by a fix — all one family: a line-oriented reader inferring YAML structure from indentation and character classes. Closed at the **class** level for the one direction that can fail open (`branches-ignore:`, the reader's only must-NOT-contain test): a token carrying a byte outside the ref/glob set is now REFUSED rather than searched, so a future tokenizer defect there yields a red or a refusal, never a silent accept. A real YAML parse was evaluated and rejected on measured evidence — see D-157 | **[autonomous]** | — | three reviewer-demonstrated residuals, all in the same three files, all deliberately left unfixed on their own lanes because fixing one of three identical checks would have been misleading |
| GATES3-F01 | ✅ **DONE (D-174)** — `hack/audit/README.md` published the AUD2 control floor as **"38 full, 32 text-only"** while the code pinned **57 / 51** (measured, not copied from this row: `CONTROL_FLOOR` in `hack/audit/aud2_exitgate_test.sh`, and a full run emits exactly 57 controls / a `--text-only` run exactly 51). The row's own question is answered **yes**: the floor is now **asserted against** the README, not restated in it. `check_readme_floor` parses the published sentence and reds if either integer disagrees with the pinned floor, so the two can no longer move apart in either direction; four mutation controls cover both integers drifting and both vacuity shapes (the claim reworded out of the check's reach, and a second copy added), which is what took the floor itself 57/51 → **61/55** | **[autonomous]** | — | found *by* the GATES3 lane's final review, deliberately NOT fixed there: `hack/audit/README.md` is outside that lane's fence, and a silent edit to a published number is the same D-124 species as the drift itself — a doc claim nothing grades. Correcting the number alone would have reset the clock, which is why the assertion, not the correction, is the fix |
...
| BASH32-F01 | **DONE** — three `hack/**` gate scripts used bash 4+ features with no version guard; under stock macOS `/bin/bash` 3.2 `hack/docs/truthlag_pins_test.sh` died at its `declare -A`, skipped its final `OK:` banner and **exited 0**, so `task docs-gates` (and `task check`) read a gate that certified almost nothing as green. Fixed by a shared, per-script-parameterised `hack/lib/require-bash.sh` floor (4.0 / 4.0 / 4.4), enforced by a new `hack/lint/bash_version_guard_test.sh` wired as a `task check` stage of its own (D-154; no ordinal — this row published "stage 20" while the stage measured **18**, and D-174's insertion has since moved it again, which is the whole argument for not quoting one) | **[autonomous]** | — | found *by* the AUD2 wave's macOS runs, not by any audit: CI is ubuntu/bash 5, so the hole is LOCAL-only and no CI lane could have surfaced it |
| CHANGELOG-CHURN | **DONE (D-180)** — every changelog-relevant commit made `main`'s committed `CHANGELOG.md` stale, because the drift gate diffed the whole render, `## Unreleased` included. Lanes absorbed it with a regenerate-and-amend step; Dependabot could not, so every bot merge reddened `main` and needed a hand-made regen PR (#131, #132). `CHANGELOG.md` now commits released versions only: `cliff.toml`'s template skips the unreleased release under `ASSENT_CHANGELOG_RELEASED_ONLY=1`, set only by the new `hack/release/render-changelog.sh`, which both `task changelog-write` and `verify-changelog.sh` go through. The gate reds only on a hand edit, a history-re-rendering `cliff.toml` change, or an unstamped tag. `changelog_gate_test.sh` §10 proves every case at both polarities, including GREEN after an ordinary commit, against a pre-D-180 mutant that reds. Follow-up **D-181**: the unstamped tag is tolerated on the tagged commit itself (only its own section missing), so a `verify` run on the tagged SHA can no longer lock the tag out of REQ-AUD-S03-01; §10c pins it. |
| LOCALE-F01 | **DONE (D-179)** — `task check` was **red on a clean `main`** for a developer with `LANG=en_US.UTF-8` and green on CI's C.UTF-8: `release-changelog-gate-test` §7a died with `comm: file 1 is not in sorted order` (uutils 0.8.0 `sort` collates by locale, its `comm` checks bytes). A grep of `hack/**` found 36 unpinned `sort`/`comm` calls in nine scripts, incl. a latent GNU-shape bug in `truthlag_pins_test.sh` (`LC_ALL=C sort` into an unpinned `comm`). All pinned `LC_ALL=C` per call; new `hack/lint/locale_pin_test.sh` (`task check` stage + PR-visible `verify` step) enforces it, with population, mutation and a host-behavioural control that fails rather than skips on CI. |
| BASH32-N5N6 | **DONE (D-169)** — two residuals in `hack/lint/bash_version_guard_test.sh`, filed by review against the D-154 gate, **both confirmed by measurement before anything was written**. **N5 — a sentinel that proved the wrong thing**: the four `PATTERN_NONPROBES` absence assertions were partnered by a `hack/sentinel.sh` fixture, which proves only that the fixture ROOT was walked — never that `probe.sh` itself was enumerated. Measured on a scratch copy of the gate: writing *only* the probe one directory outside the walked root, and separately naming it `probe.txt` so `find -name '*.sh'` never lists it, EACH left all four assertions green, **0 FAILs, exit 0, 57 PASS lines** — byte-identical to the honest run. Closed by factoring the population out of `scan_features` into `enumerate_scripts`, so absence is now asserted about a path the SAME `find` is shown to have LISTED (exact match), with non-empty guards on both the enumeration and the fixture path plus an existence check on the file; `MUST_BE_CLEAN` gets the same treatment, its `[ -f ]` partner having proved existence rather than membership. A new enumeration control pins both misdirections directly, in both directions. **N6 — a completeness claim outrunning its sweep**: the header's "GUARD SHAPES NOT RECOGNISED" block said *"If you hit that message on a file you know is guarded, this list is why"*, which reads as exhaustive. Every individual claim in it was TRUE; only the framing overreached — `case "${BASH_VERSINFO[0]}" in 0\|1\|2\|3) exit 1;; esac` is a real guard, is rejected (measured), and was unlisted, as are `$BASH_VERSION`-reading guards, line-split guards, `=~` tests and `${BASH_VERSINFO:0:1}`. **Widening the sweep was rejected as the fix**: the rejected set is *every spelling that is not the one accepted spelling*, so no enumeration of it can ever be complete and a wider list would restate the same false claim. Instead the block states the ACCEPTED rule and the named shapes moved into `GUARD_SHAPES_UNRECOGNISED`, graded end-to-end through `grade_root` by control (i) — each claim kept true by a test, and the completeness claim no longer made. **Round 2, found by independent review of this very change:** the replacement prose committed the same overreach one corner smaller — it asserted "THAT RULE IS THE COMPLETE STATEMENT" while omitting the predicate's `^[^#]*` anchor, so `[ $# -gt 0 ] && [ "${BASH_VERSINFO[0]}" -lt 4 ] && exit 1` satisfied the stated rule verbatim and was rejected anyway (measured). Prose restating a regexp is a DERIVED claim and cannot certify itself; stating a rule instead of a list removes the enumeration problem, not the drift problem. The `#` clause is now stated in both the header and the block above `has_versinfo_guard`, both `$#` shapes are graded entries, and the header defers to the regexp as the authority. A **population-coupling** control was also added: `scan_features` must grade exactly what `enumerate_scripts` lists, pinned in both the subset and superset directions so neither a wider nor a narrower divergent `find` can dissolve the N5 fix. **Round 3:** that control itself had a NO-VERDICT branch — an order-sensitive equality diagnosed by set membership, so a same-set-different-order `find` (`sort -r`) emitted neither PASS nor FAIL (measured: EXIT=0, 70 PASS, 0 FAIL, zero coupling lines) and, with no total-PASS-count pin anywhere in the gate, read exactly like a satisfied control. Closed at the class level: both sides sorted (order is not part of the invariant; plain `sort`, so duplicates still differ), AND the else-branch made total — a difference the per-file loops cannot name is itself a FAIL. Also: the header rule now names the operator set, and the per-file loops moved from unquoted `for` to `while IFS= read -r` | **[autonomous]** | BASH32-F01 (D-154) | filed by an independent reviewer on a prior lane and deliberately left there: both are defects in the CHECKER, not in any checked file (every `hack/**` script is byte-unchanged), and `hack/lint/**` was held by a concurrent lane. N6 is the **third** occurrence in a single session of the D-157/D-160/D-164/D-165 species — a completeness claim outrunning the sweep behind it |
| UNIV-COSIGN | **DONE (D-160)** — `hack/release/install_cosign_pin_test.sh` graded the D-153 signer pin **existentially** ("*some* `cosign verify-blob` in this file is pinned") where the published guarantee is **universal**, so a **second, unpinned** invocation passed: measured before the fix on scratch copies of all three graded files, rc=**0**/**0**/**0**. `has_flag` grepped the whole folded extraction so a pinned sibling satisfied it, `one_value`'s `sort -u` collapsed the agreeing values, §0 printed the invocation count without asserting it, and 4b/5d's stub-log greps were positive-only — the same shape at runtime. `SECURITY.md` had **no invocation-level grading at all**, only the drift comparison. Closed by one predicate (`pin_violations`) applied to **every** invocation in **all three** files plus its runtime twin, with fail-CLOSED classification of anything it cannot place, mutation-proven by eight mutants per file chosen for **branch** coverage (second-unpinned / second-pinned / off-command-position / no-candidates / fold-broken / no-invocations / empty-identity-regexp / foreign-issuer) plus a prose-exemption control. The check compares each invocation's **value** against SECURITY.md's published pair, not just flag presence: `--certificate-identity-regexp ''` carries the flag, matches every Fulcio identity, and slips past `one_value`'s `sort -u` because an empty capture is not a line. **CORRECTION TO THE RECORDED REMEDY — do not implement it:** the filed remedy "assert the extracted invocation count is exactly 1 per graded file" is **factually wrong and would red `main` on landing**, because `SECURITY.md` legitimately publishes **two** invocations (`SECURITY.md:86` over the archive and `:93` over `checksums.txt`, which is what covers the SBOMs listed inside it). The property is universal, not a count. **Second round (UC-01..UC-04), caught by independent review:** the first fix reproduced the same existential defect one scope down — per **line** instead of per **file** — so an unpinned call CHAINED onto a pinned one (`&&`, `;`, a pipe, or backslash-folded) passed, and the **full gate exited 0** on `SECURITY.md`, which has no runtime twin. Closed by per-**occurrence** grading (`MULTI-OCCURRENCE`), by comparing **every** value of every flag rather than the greedy-last (an empty regexp placed first was masked by a correct one placed second), and by diagnosing quote style separately (`UNQUOTED-VALUE`) so a correctly pinned double-quoted call is accepted instead of being refused with a false "empty regexp" reason. **Third round (UC-05..UC-09):** `occurrence_count` read *absence of a second token* as *proof of singularity*, so `cosign "verify-blob"` / `cosign 'verify-blob'` / `cosign verify-blob""` hid a chained unpinned call — **rc=0 on all three files at whole-gate level** against the round-2 gate (`A && B` short-circuits, so `set -e` never fires and even the runtime twin missed it). Fixed by stripping shell quotes before the compare, **explicitly a spelling patch, not a terminator**: a structural terminator was ruled out with evidence, because both scripts legitimately end their real invocation with an or-else `die` tail, so separator-based refusal would red the real files. Since completeness is unavailable, **the `PASS:` banner no longer claims it** — it now reports OBSERVED counts per file, lists only ASSERTED properties (each mutation-proven), and states the NOT-ASSERTED residual in the same breath, so an unknown fifth spelling leaves the gate *incomplete* rather than *wrong*. **R3-01, found by review in the very paragraph that added that banner:** the backticked-mention exemption keys on the substring appearing **anywhere on the line** and is tested first, so a line carrying a backticked mention AND live calls is exempted — `pin_violations` empty, whole gate rc=0. Not patched (narrowing it needs the markdown/quote parsing that produced UC-01/02/03); **disclosed as the FIRST and most reachable residual in the banner**, with exempted lines no longer described as "prose". Also: MULTI-OCCURRENCE now says TOKENS and names trailing comments as a cause (it was false as printed), and SC1087 silenced | **[autonomous]** | AUD2-F01, D-153 (the pinned value) | demonstrated by a reviewer *on* the AUD2-F01 lane and out of its scope — F01 added a third graded file to a gate whose quantifier was already wrong, so fixing the quantifier is a change to the checker, not to any checked file (all three are byte-unchanged) |
...
as a `task check` stage — wired **PR-visibly** in the `verify` job, not only in the
...
stands. It added no stage: `CHECK_STAGES` was unchanged *by that fix*, because it extends the
existing `release-install-cosign-pin-test` gate instead of starting a second published truth
(D-128). **No ordinal or array length is quoted in this paragraph any more (DOCTRUTH / D-174):**
it used to say "the 19th `task check` stage" and "unchanged at 19", with a trailing note that the
array "has since moved on … to 21" — and the stage measured **21** by then and **22** after D-174,
so the disclaimer aged into a third wrong number rather than saving the first two. All three traps above were handled: the new sections are offline against the same
...
recipes, not a `task check` stage, so that work adds no `CHECK_STAGES` entry either, whatever the array's current length — see the DOCTRUTH note above.
...
| DOCSNAV-S02 | ✅ **DONE (D-174)** — S01 armed the gate in CI **only**, so the invariant had no local pre-merge evidence: `task check` was green over a page in neither list and `main` reddened on push, taking the Pages deploy with it. `docs-build` is now a `check:` stage and pinned in `CHECK_STAGES` in the same commit (the array moved 21 → 22). Same `mkdocs build --strict`, same `mkdocs.yml` — one implementation, local and CI, so the two halves cannot skew | **[autonomous]** | S01 | an unlisted docs page reds `task check` locally, at the `docs-build` stage, naming the page |
