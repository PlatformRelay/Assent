...
| E11-S01 | ✅ **DONE (D-156)** — tier-1 (CEL) ceiling recorded: `docs/planning/rego-tier-ceiling.md` | **[autonomous]** | none | **E11 NARROWED**: cross-manifest + set-difference **struck** (both unconditional). **Fold/aggregate is the only UNCONDITIONAL justification left**; the graph shape waits on OQ-35/OQ-36 for an in-contract adjacency. Cross-manifest is an *input* limit S05's identical `EvaluationInput` does not lift |
