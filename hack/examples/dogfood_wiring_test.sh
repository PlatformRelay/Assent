...
V="$(step_key_fixture step-if-bare 'if: false' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 8 "$V" "$SELF_REL" "a step disarmed with a BARE 'if:' is DISARMED (the control that always worked)"

V="$(step_key_fixture step-if-spaced 'if : false' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 8 "$V" "$SELF_REL" "a step disarmed with 'if : ' — a space before the colon — is DISARMED (G5-02: rc=0 at 01b2322; a conformant parser reads it as 'if')"

V="$(step_key_fixture step-if-quoted '"if": false' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 8 "$V" "$SELF_REL" "a step disarmed with a QUOTED '\"if\":' is DISARMED (G5-02)"

V="$(step_key_fixture step-coe-spaced 'continue-on-error : true' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 8 "$V" "$SELF_REL" "a step neutered with 'continue-on-error : ' is DISARMED (G5-02)"

V="$(job_key_fixture job-if-spaced 'if : false' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 4 "$V" "$SELF_REL" "a JOB carrying 'if : ' has release-exitgate's push-only guard — rc=4, not 0 (G5-02: this is the RELSE-08 shape the whole lane exists to catch)"

V="$(job_key_fixture job-if-quoted '"if": false' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 4 "$V" "$SELF_REL" "a JOB carrying a quoted '\"if\":' is likewise rc=4 (G5-02)"

V="$(job_key_fixture job-needs-quoted '"needs": release-exitgate' hack/examples/dogfood_wiring_test.sh)"
helper_wired_is 9 "$V" "$SELF_REL" "a JOB carrying a quoted '\"needs\":' is rc=9 — skipped dependency, skipped job (G5-02)"
...
V="$WORK/fixture.step-ordinary-keys.yaml"
{
  printf 'name: f\non:\n  pull_request:\njobs:\n  verify:\n    runs-on: ubuntu-latest\n    timeout-minutes: 60\n    steps:\n'
  printf '      - name: gate\n        id: g\n        shell: bash\n        env:\n          FOO: bar\n'
  printf '        run: bash hack/examples/dogfood_wiring_test.sh\n'
} >"$V"
helper_wired_is 0 "$V" "$SELF_REL" "a step carrying ordinary keys (id:, shell:, env:) in a job with timeout-minutes: is still WIRED — the key allowlist grades disarms, it does not refuse normal structure (G5-02, other polarity)"
